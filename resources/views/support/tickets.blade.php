...
<div class="mx-auto max-w-7xl">
    ...
    <form method="GET" class="mt-6 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_180px_auto]"><input name="q" value="{{ request('q') }}" placeholder="Search reference, church, or subject" class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"><select name="status" class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm"><option value="">All statuses</option>@foreach(['new','triaged','in_progress','waiting_on_church','resolved','closed'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ str($status)->headline() }}</option>@endforeach</select><button class="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white">Filter</button></form>
        ...
        <div class="space-y-4">
            @forelse($tickets as $ticket)
                <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    @if($ticket->replies->isNotEmpty())<div class="mt-4 space-y-2">@foreach($ticket->replies->sortBy('created_at') as $reply)<div class="rounded-xl {{ $reply->is_internal ? 'bg-amber-50' : 'bg-slate-50' }} p-3 text-xs"><div class="font-bold">{{ data_get($reply->author, 'name', 'Support') }} @if($reply->is_internal)<span class="font-normal text-amber-700">· Internal</span>@endif</div><p class="mt-1 text-slate-600">{{ $reply->body }}</p></div>@endforeach</div>@endif
